...
const u32 mlu580_attr_info[CN_MLU580_MAX][ATTR_TYPE_NUM] = {
	/* MLU580_EVB */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM10, 0x5400000},
	/* MLU560 */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM10, 0x4600000},
	/* MLU560_F */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM10, 0x4600000},
	/* MLU580 */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM12, 0x5400000},
	/* MLU570 */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM10, 0x4600000},
	/* MLU570F */
	{QUAD_CNT1, UNION_PER_QUAD8, ATTR_SUPPORT, MAX_CLUSTER_NUM10, 0x4600000},
...
const u32 mlu580_attr_cap_info[ATTR_MAX_CAP][COMPUT_MAX] = {
	/* computing cap */
	/* major      minor */
	...
	{ATTR_MLU580_ISA, ATTR_SUPPORT},
	/* memory cap */
	/* l1 cache 256KB   l2 cache      sharemem */
	{0x40000, ATTR_SUPPORT, ATTR_NOT_SUPPORT},
};

const u32 mlu580_board_vf_info[BOARD_VF_NUM] = {
	/* ipu core num */
	IPUCORE_NUM4,
	/* ddr freq */
	DDR_FREQ1600
...
void fill_computing_cap_mlu580(void *pcore)
{
	...
	pcompute_cap->major = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_MAJOR];
	pcompute_cap->minor = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_MINOR];
	pcompute_cap->sparse = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_SPARSE];
	pcompute_cap->fp16 = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_FP16];
	pcompute_cap->int4 = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_INT4];
	pcompute_cap->int8 = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_INT8];
	pcompute_cap->bf16 = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_BF16];
	pcompute_cap->tf32 = mlu580_attr_cap_info[ATTR_COMPUTING][COMPUT_TF32];
}

void fill_heterogeneous_cap_mlu580(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	...
	pheterogeneous_cap->tiny_core =
		mlu580_attr_info[subtype][TINY_CORE];
	pheterogeneous_cap->codec_jpeg =
		mlu580_attr_cap_info[ATTR_HETEROG][HETEROG_JPEG];
	pheterogeneous_cap->codec_h264 =
		mlu580_attr_cap_info[ATTR_HETEROG][HETEROG_H264];
	pheterogeneous_cap->codec_h265 =
		mlu580_attr_cap_info[ATTR_HETEROG][HETEROG_H265];
	pheterogeneous_cap->isp_core = 0;
}

void fill_elastic_cap_mlu580(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	struct cn_board_info *pboardi = &core->board_info;
	...
	pelastic_cap->max_cluster_count_per_union_task =
		mlu580_board_info[subtype][INFO_KC_LIMIT];
	pelastic_cap->o_max_cluster_count_per_union_task =
		mlu580_board_info[subtype][INFO_O_KC_LIMIT];
	...
	pelastic_cap->max_quadrant_count =
		mlu580_attr_info[subtype][QUADRANDT_COUNT];
	pelastic_cap->max_union_type_per_quadrant =
		mlu580_attr_info[subtype][UNIONT_PER_QUADRANT];
	pelastic_cap->mlu_isa_version =
		mlu580_attr_cap_info[ATTR_ELASTIC][ELASTIC_ISA];
	pelastic_cap->is_multiple_tensor_processor =
		mlu580_attr_cap_info[ATTR_ELASTIC][ELASTIC_IS_MULT_TP];
}

void fill_memory_cap_mlu580(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	struct cn_board_info *pboardi = &core->board_info;
	...
	pmem_cap->max_l2_cache_size =
		mlu580_board_info[subtype][INFO_CACHE_SIZE];
	...
	pmem_cap->cluster_l1_cache_support =
		mlu580_attr_cap_info[ATTR_MEMORY][MEM_CLUSTER_LI_CACHE];
	pmem_cap->max_persisting_l2_cache_size =
		mlu580_attr_info[subtype][ATTR_PERSIS_L2_CACHE];
	pmem_cap->max_shared_memory_size_per_union_task =
		mlu580_attr_cap_info[ATTR_MEMORY][MEM_MAX_SHARE_MEM];

	pmem_cap->can_use_host_pointer_for_register_mem = 1;
	pmem_cap->can_map_host_memory = 1;
}

void fill_hardware_cap_mlu580(void *pcore)
{
	...
	phard_cap->ecc_support = mlu580_board_info[subtype][INFO_ECC_SUPPORT];
	phard_cap->cluster_clock_rate =
		mlu580_board_info[subtype][INFO_MAX_IPU_FREQ] * 1000;
	phard_cap->memory_clock_rate = pboardi->ddr_freq * 1000;
	phard_cap->bus_width = mlu580_board_info[subtype][INFO_BUS_WIDTH];
...
void fill_boardinfo_mlu580_vf(struct cn_core_set *core)
{
	...
	pboardi->stack_size = mlu580_board_info[subtype][INFO_STACK_SIZE];
	pboardi->sram_size = mlu580_board_info[subtype][INFO_SRAM_SIZE];
	pboardi->rated_ipu_freq = mlu580_board_info[subtype][INFO_MAX_IPU_FREQ];
	pboardi->ipu_core_num = mlu580_board_vf_info[ATTR_IPU_CORE];
	pboardi->ddr_freq = mlu580_board_vf_info[ATTR_DDR_FREQ];
}

void fill_attribute_mlu580(struct cn_core_set *core)
{
	/* Computing Capabilities */
	fill_computing_cap_mlu580(core);

	/* Heterogeneous Capabilities */
	fill_heterogeneous_cap_mlu580(core);

	/* Elastic Capabilities */
	fill_elastic_cap_mlu580(core);

	/* Memory Capacities */
	fill_memory_cap_mlu580(core);

	/* Hardware Proterties */
	fill_hardware_cap_mlu580(core);
}

void fill_elastic_cap_mlu580_vf(struct cn_core_set *core)
{
	struct cn_elastic_cap *pelastic_cap = NULL;
	struct cndev_attr_set *attr_set = NULL;
	...
	pelastic_cap->mlu_isa_version = mlu580_attr_cap_info[ATTR_ELASTIC][ELASTIC_ISA];
	pelastic_cap->is_multiple_tensor_processor = mlu580_attr_cap_info[ATTR_ELASTIC][ELASTIC_IS_MULT_TP];
}

void cn_attr_fill_memory_cap_mlu580_vf(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	struct cn_board_info *pboardi = &core->board_info;
	struct cn_memory_cap *pmem_cap = NULL;
	...
	pmem_cap = &attr_set->attr_info.memory_cap;
	pmem_cap->cluster_l1_cache_support = mlu580_attr_cap_info[ATTR_MEMORY][MEM_CLUSTER_LI_CACHE];
...
void fill_attribute_mlu580_vf(struct cn_core_set *core)
{
	...
	/* Elastic Capabilities */
	fill_elastic_cap_mlu580_vf(core);

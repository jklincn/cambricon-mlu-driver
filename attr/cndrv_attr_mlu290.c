...
const u32 mlu290_attr_info[CN_MLU290_MAX][ATTR_TYPE_NUM] = {
	/* MLU290 */
	{QUAD_CNT4, UNION_PER_QUAD4, ATTR_NOT_SUPPORT, MAX_CLUSTER_NUM16, 0},
	/* MLU290_VF */
	{QUAD_CNT4, UNION_PER_QUAD4, ATTR_NOT_SUPPORT, MAX_CLUSTER_NUM16, 0},
	/* UNKNOWN */
	{QUAD_CNT4, UNION_PER_QUAD4, ATTR_NOT_SUPPORT, MAX_CLUSTER_NUM16, 0},
...
const u32 mlu290_attr_cap_info[ATTR_MAX_CAP][COMPUT_MAX] = {
	/* computing cap */
	/* major      minor */
...
void fill_computing_cap_mlu290(void *pcore)
{
	...
	pcompute_cap->major = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_MAJOR];
	pcompute_cap->minor = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_MINOR];
	pcompute_cap->sparse = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_SPARSE];
	pcompute_cap->fp16 = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_FP16];
	pcompute_cap->int4 = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_INT4];
	pcompute_cap->int8 = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_INT8];
	pcompute_cap->bf16 = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_BF16];
	pcompute_cap->tf32 = mlu290_attr_cap_info[ATTR_COMPUTING][COMPUT_TF32];
}

void fill_heterogeneous_cap_mlu290(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	...
	pheterogeneous_cap->tiny_core = mlu290_attr_info[subtype][TINY_CORE];
	pheterogeneous_cap->codec_jpeg = mlu290_attr_cap_info[ATTR_HETEROG][HETEROG_JPEG];
	pheterogeneous_cap->codec_h264 = mlu290_attr_cap_info[ATTR_HETEROG][HETEROG_H264];
	pheterogeneous_cap->codec_h265 = mlu290_attr_cap_info[ATTR_HETEROG][HETEROG_H265];
	pheterogeneous_cap->isp_core = 0;
}

void fill_elastic_cap_mlu290(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	struct cn_board_info *pboardi = &core->board_info;
	...
	pelastic_cap->max_cluster_count_per_union_task =
		mlu290_board_info[subtype][INFO_KC_LIMIT];
	pelastic_cap->o_max_cluster_count_per_union_task =
		mlu290_board_info[subtype][INFO_O_KC_LIMIT];

	pelastic_cap->max_cluster_count = pboardi->cluster_num;
	pelastic_cap->max_core_count_per_cluster = pboardi->ipu_core_num;
	pelastic_cap->max_quadrant_count = mlu290_attr_info[subtype][QUADRANDT_COUNT];
	pelastic_cap->max_union_type_per_quadrant = mlu290_attr_info[subtype][UNIONT_PER_QUADRANT];
	pelastic_cap->mlu_isa_version = mlu290_attr_cap_info[ATTR_ELASTIC][ELASTIC_ISA];
	pelastic_cap->is_multiple_tensor_processor = mlu290_attr_cap_info[ATTR_ELASTIC][ELASTIC_IS_MULT_TP];
}

void fill_memory_cap_mlu290(void *pcore)
{
	struct cn_core_set *core = (struct cn_core_set *)pcore;
	struct cn_board_info *pboardi = &core->board_info;
	...
	pmem_cap->max_l2_cache_size =
		mlu290_board_info[subtype][INFO_CACHE_SIZE];
	...
	pmem_cap->cluster_l1_cache_support =
		mlu290_attr_cap_info[ATTR_MEMORY][MEM_CLUSTER_LI_CACHE];
	pmem_cap->max_persisting_l2_cache_size =
		mlu290_attr_info[subtype][ATTR_PERSIS_L2_CACHE];
	pmem_cap->max_shared_memory_size_per_union_task =
		mlu290_attr_cap_info[ATTR_MEMORY][MEM_MAX_SHARE_MEM];
}

void fill_hardware_cap_mlu290(void *pcore)
{
	...
	phard_cap->ecc_support = mlu290_board_info[subtype][INFO_ECC_SUPPORT];
	phard_cap->cluster_clock_rate =
		mlu290_board_info[subtype][INFO_MAX_IPU_FREQ] * 1000;
	phard_cap->memory_clock_rate = pboardi->ddr_freq * 1000;
	phard_cap->bus_width = mlu290_board_info[subtype][INFO_BUS_WIDTH];
...
void fill_attribute_mlu290(struct cn_core_set *core)
{
	...
	fill_computing_cap_mlu290(core);

	/* Heterogeneous Capabilities */
	fill_heterogeneous_cap_mlu290(core);

	/* Elastic Capabilities */
	fill_elastic_cap_mlu290(core);

	/* Memory Capacities */
	fill_memory_cap_mlu290(core);

	/* Hardware Proterties */
	fill_hardware_cap_mlu290(core);
}

void fill_elastic_cap_mlu290_vf(struct cn_core_set *core)
{
	struct cn_elastic_cap *pelastic_cap = NULL;
	struct cndev_attr_set *attr_set = NULL;
	...
	pelastic_cap->mlu_isa_version =
		mlu290_attr_cap_info[ATTR_ELASTIC][ELASTIC_ISA];
	pelastic_cap->is_multiple_tensor_processor =
		mlu290_attr_cap_info[ATTR_ELASTIC][ELASTIC_IS_MULT_TP];
}
...
void fill_attribute_mlu290_vf(struct cn_core_set *core)
{
	...
	/* Elastic Capabilities */
	fill_elastic_cap_mlu290_vf(core);

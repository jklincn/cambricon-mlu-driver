...
extern struct cn_pci_info c20_pci_info;
extern struct cn_pci_info c20_vf_pci_info;
extern struct cn_pci_info ce3226_pci_info;

static const struct pci_device_id cn_pci_ids_mlu290_ce3226[] = {
	CAMBR_DEVICE(cambricon_dm_VID, CN_C20_DID, c20_pci_info),
	CAMBR_DEVICE(cambricon_dm_VID, CN_C20_VF1_DID, c20_vf_pci_info),
	CAMBR_DEVICE(cambricon_dm_VID, CN_CE3226_DID, ce3226_pci_info),
	{}
};

static const struct pci_device_id cn_pci_sriov_ids_mlu290_ce3226[] = {
	CAMBR_DEVICE(cambricon_dm_VID, CN_C20_DID, c20_pci_info),
	CAMBR_DEVICE(cambricon_dm_VID, CN_CE3226_DID, ce3226_pci_info),
	{}
};
...
MODULE_DEVICE_TABLE(pci_cn_pci_ids_mlu290_ce3226, cn_pci_ids_mlu290_ce3226);
MODULE_DEVICE_TABLE(pci_cn_pci_sriov_ids_mlu290_ce3226, cn_pci_sriov_ids_mlu290_ce3226);

#define DRV_MODULE_NAME		"cambricon-pci-drv_mlu290_ce3226"
...
static int cn_pci_get_dma_channel_info(void *pcie_priv, struct dma_channel_info_s *dma_channel_info)
{
	...
	for (index = 0; index < pcie_set->max_phy_channel; index++) {
		if (pcie_set->running_channels[index])
			++dma_channel_info->phy_channel;
	}

	for (order = pcie_set->max_desc_order; order >= 0; order--) {
		list = pcie_set->order_table[order].list;
		cnt = pcie_set->order_table[order].number;

		for (index = 0; index < cnt; index++) {
			if (list[index]->status != CHANNEL_IDLE)
...
static struct pci_driver cn_pci_driver = {
	...
	.id_table = cn_pci_ids_mlu290_ce3226,
	.probe = cn_pci_probe,
...
__attribute__((unused)) static struct pci_driver cn_pci_sriov_driver = {
	.name = DRV_MODULE_NAME,
	.id_table = cn_pci_sriov_ids_mlu290_ce3226,
	.probe = cn_pci_probe,
	.remove = cn_pci_remove,
	.suspend = cn_pci_suspend,
	.resume = cn_pci_resume,
...
int cn_pci_drv_init_mlu290_ce3226(void)
{
	return pci_register_driver(&cn_pci_driver);
}

void cn_pci_drv_exit_mlu290_ce3226(void)
{

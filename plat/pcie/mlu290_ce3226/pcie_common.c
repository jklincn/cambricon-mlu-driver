...
__attribute__((unused)) static int pcie_register_bar(struct cn_pcie_set *pcie_set)
{
	...
	list_for_each_entry(bar, &pcie_set->bar_resource_head, list) {
		cn_dev_debug("bar_type=%d, bar_index=%d, bar.base=%p, bar.sz=%llx",
...
static int cn_pci_get_idle_phy_channel(struct pcie_dma_task *task)
{
	int phy;
	ulong mask;
...
static int cn_pci_task_dma_start(struct pcie_dma_task *task)
{
	...
	phy = cn_pci_get_idle_phy_channel(task);
	if (phy == -1)
		return 0;
	...
	__sync_lock_test_and_set(&pcie_set->running_channels[phy], (unsigned long)channel);
...
static int cn_pci_task_fair_schedule(struct cn_pcie_set *pcie_set)
{
	...
	while (1) {
		...
		/* if all phy channels are running, break */
		ulong mask = (ulong)(pcie_set->dma_phy_channel_mask &
				(~pcie_set->channel_run_flag));
		if (!mask)
			break;
			...
			int status, struct cn_pcie_set *p_set)
{
	...
	if ((channel->status != CHANNEL_RUNNING) || (!task)) {
		cn_dev_pcie_err(p_set, "status:%d phy_channel:%d",
				channel->status, phy_channel);
		return;
	}
	...
	if (p_set->af_enable && (!task->dma_copy) &&
		(channel->status == CHANNEL_COMPLETED || channel->status == CHANNEL_ASYNC_FREE)) {
		ret = kfifo_in(&p_set->af_fifo, &channel, sizeof(channel));
		if (ret != sizeof(channel))
			cn_dev_pcie_err(p_set, "af_fifo kfifo_in fail");
...
static int cn_pci_link_eq_set(void *pcie_priv)
{
	...
	cn_dev_info("set parent link eq PCI_SEC_LNKCTL3 %#x", lnk_eq);
	return 0;
}
...
static int cn_pci_retrain_set(void *pcie_priv)
{
	...
	cn_dev_info("set parent PCI_EXP_LNKCTL %#x", lnk_ctrl);
	return 0;
}
...
retry:
		...
		if (!cn_pci_wait_for_pending_transaction(pdev))
			cn_dev_err("timed out waiting for pending transaction");
...
static int cn_pci_set_cspeed(unsigned int cspeed, void *pcie_priv)
{
	...
	cn_dev_info("PCIe target speed is %s", PCIE_SPEED_STR(cspeed));

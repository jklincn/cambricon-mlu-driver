...
static int cn_pci_dma_try_p2p(struct cn_pcie_set *src, struct cn_pcie_set *dst)
{
	...
	int ret = 0;
	int able = 0;
	struct mem_attr src_mm;
	struct mem_attr dst_mm;
...
static size_t cn_pci_dma_p2p(struct peer_s *peer)
{
	...
	task = cn_pci_get_dma_idle_task(dst, DMA_P2P);
	if (!task)
		return ret;

	if (dst->ops->dma_align) {
		size_t head_cnt = 0;
		size_t tail_cnt = 0;
		...
		t_align.ia = dst_addr;
		t_align.size = count;
		if (dst->ops->dma_align(&t_align, &head_cnt, &tail_cnt)) {
			ret = cn_pci_dma_p2p_exchange(peer);
			/* set transfer direction for put dma idle task */
	...
	while (count) {
		...
		/* set src window and calculate size */
		base = src->ops->set_bar_window(src_addr, bar_src, src);
		offset = src_addr - base;
		size = bar_src->size - offset;
		len = min_t(size_t, count, size);

		/* init transfer task */
		transfer.d_ipu = dst_addr;
		transfer.d_bar = bar_src->bus_base + offset;
		transfer.size = len;
		transfer.direction = DMA_P2P;

		/* use src dma for transfer */
		if (cn_pci_init_dma_task(task, &transfer, PCIE_DMA_P2P, dst)) {
			cn_dev_pcie_err(src, "p2p task init fail");
			ret = count;
			goto put_bar;
		}

		task->pcie_set_dst = src;
...
put_task:
	cn_pci_put_dma_idle_task(dst, task);

...
static irqreturn_t pcie_dma_interrupt_handle(int index, void *data)
{
	...
	if (cn_is_mim_en_bdf(pcie_set->bdf, pcie_set->pdev->is_virtfn)) {
		channel_mask = pcie_set->dma_res.channel_mask;
...
static int pci_hw_msix_enable(struct cn_pcie_set *pcie_set)
{
	...
	cn_pci_reg_write32(pcie_set, GIC_MSIX_VECTOR_COUNT, (MSIX_COUNT == 256)?5:0);
...
static void desc_write64(unsigned long offset, u64 val,
	struct dma_channel_info *channel)
{
...
static void desc_write32(unsigned long offset, u32 val,
	struct dma_channel_info *channel)
{
...
static void pcie_show_desc_list(struct dma_channel_info *channel)
{
	...
	cn_dev_pcie_err(channel->pcie_set, "pcie channel:%d len:%ld desc_len:%d",
		channel->id, channel->transfer_length, channel->desc_len);
						...
						u64 status, u64 mask, int i)
{
	...
	while (status) {
		...
		if (pcie_set->irq_desc[index].handler) {
			start = get_jiffies_64();
			pcie_set->irq_desc[index].handler(index,
				pcie_set->irq_desc[index].data);
			end = get_jiffies_64();
			...
			cn_dev_pcie_err(pcie_set,
				"no interrupt handle!:%llx %x", status, index);
	}
	pcie_set->do_dma_irq_status = 0;
}
...
static int pcie_init(struct cn_pcie_set *pcie_set)
{
	...
	for (i = 0; i < pcie_set->max_channel; i++) {
		...
		cn_dev_pcie_debug(pcie_set, "channel%d: desc_addr=%llx", i, desc_addr);

		for (j = 0; j < channel->desc_size / 8; j++)
			desc_write64(j * 8, 0, channel);

		for (j = 0; j < channel->desc_size / DESC_SIZE; j++) {
			desc_addr += DESC_SIZE;
			desc_write32(j * DESC_SIZE + DE_NDL,
					LOWER32(desc_addr), channel);
			desc_write32(j * DESC_SIZE + DE_NDU,
					UPPER32(desc_addr), channel);
		}

		channel->desc_buf = cn_kzalloc(channel->desc_size, GFP_KERNEL);
		if (!channel->desc_buf) {
			cn_dev_pcie_err(pcie_set, "alloc desc buff error.");
			goto nomem;
		}
		desc_addr = channel->desc_device_va;
		for (j = 0; j < channel->desc_size / DESC_SIZE; j++) {

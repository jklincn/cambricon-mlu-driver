...
static int cn_pci_dma_phy_empty(struct cn_pcie_set *pcie_set, int phy)
{
	...
	for (i = 0; i < pcie_set->dma_set.dma_fetch_buff; i++) {
		if (pcie_set->dma_set.running_channels[phy][i]) {
			ret = 0;
			break;
		}
	}
		...
		struct pcie_dma_task *task, int order)
{
	...
	for (n = order; n <= pcie_set->dma_set.max_desc_order - 1; n++) {
	...
	struct cn_pcie_set *pcie_set, struct pcie_dma_task *task, int order)
{
	...
	if (!pcie_set->dma_set.shared_channel_list)
		return NULL;

	for (i = 0; i < pcie_set->dma_set.shared_channel_cnt;  i++) {
		index = (i + pcie_set->dma_set.shared_channel_search) % pcie_set->dma_set.shared_channel_cnt;
		channel = pcie_set->dma_set.shared_channel_list[index];
		if (__sync_bool_compare_and_swap(
					&channel->status,
					CHANNEL_IDLE, CHANNEL_ASSIGNED)) {
			__sync_fetch_and_add(&pcie_set->dma_set.shared_channel_search, 1);
			channel->task = task;
			return channel;
		}
	}
		...
		struct pcie_dma_task *task, int nents)
{
	...
	cap = cap ? cap : roundup_pow_of_two(
		(pcie_set->dma_set.dma_buffer_size / PAGE_SIZE) + 1);
	if (nents <= 0 || nents > cap) {
...
cn_pci_channel_dma_ready(struct dma_channel_info *channel, size_t remain)
{
	...
	if (pcie_set->ops->fill_desc_list(channel)) {
		cn_dev_pcie_err(pcie_set, "fill desc failed");
		return -1;
	}

	channel->status = CHANNEL_READY;

	channel_mask = pcie_set->dma_set.dma_phy_channel_mask;
...
retry:
	/***
	 * Note:
	 *	For mlu590 each function has one engine which is default setting.
	 *	1. Each function has Four-Queue to do as DMA-DESP container and its depth is 8. And all the
	 *	   queue in host use the same one IRQ.
	 *	2. When support Async DMA. here select two queue to support it and it work in ARM side.
	 *	3. The time to fill full queue is shorter than engine fetch from it.
	 * Atction:
	 *	D2H use queue-0
	 *	H2D use queue-1
	 */
	...
	for (phy = phy_start; phy < phy_end; phy++) {
		if (!test_bit(phy, &channel_mask))
			continue;
		ret = cn_pci_channel_dma_start(pcie_set, channel, phy);
		if (ret < 0) {
			return -1;
		} else if (!ret) {
			start = 1;
...
after_search:
	if (start == 0) {
		...
		for (phy = 0; phy < pcie_set->dma_set.max_phy_channel; phy++) {
			if (!cn_pci_dma_phy_empty(pcie_set, phy)) {
				empty = 0;
				break;
			}
		}
		...
		if (empty && remain == 0) {
			...
			if (!kfifo_is_empty(&task->ready_fifo)) {
				cn_dev_pcie_debug(pcie_set, "####dma retry go####");
	...
	for (i = 0; i < task->nents; i++) {
		...
		if (channel->direction == DMA_H2D) {
			dma_addr = (unsigned long)dma_map_page(dev, pg, offset,
					count, DMA_TO_DEVICE);
		} else if (channel->direction == DMA_D2H) {
			dma_addr = (unsigned long)dma_map_page(dev, pg, offset,
...
static int cn_pci_get_linear_p2p_desc_num(struct cn_pcie_set *pcie_set,
			struct sg_table *p2p_sg_tb)
{
	...
	for_each_sg(p2p_sg_tb->sgl, sg, p2p_sg_tb->nents, i) {
		count = sg_dma_len(sg);
		desc_num += count / pcie_set->per_desc_max_size;
		if (count % pcie_set->per_desc_max_size)
			desc_num++;
	}
			...
			unsigned long cpu_addr, size_t len)
{
	...
	if (task->p2p_linear_remap)
		return cn_pci_get_linear_p2p_desc_num(pcie_set, task->p2p_sg_tb);
...
static int cn_pci_channel_update_sgl(struct dma_channel_info *channel)
{
	...
	if (task->p2p_linear_remap) {
		channel->nents = task->p2p_sg_tb->nents;
		channel->p2p_sgl = task->p2p_sg_tb->sgl;
	} else {
	...
	if ((task->dma_type != PCIE_DMA_P2P) ||
		(task->p2p_trans_type != P2P_TRANS_BUS_ADDRESS)) {
		if (channel->p2p_sgl) {
			nents = dma_map_sg(&pcie_set->pdev->dev, channel->p2p_sgl, channel->nents,
					DMA_BIDIRECTIONAL);
		} else if (channel->direction == DMA_H2D) {
		...
		if (!task->p2p_linear_remap) {
			sg_dma_len((struct scatterlist *)channel->sg) = channel->transfer_length;
			sg_dma_address((struct scatterlist *)channel->sg) = channel->cpu_addr;
			channel->nents = 1;
		}
	}
...
static int cn_pci_channel_dma_end(struct dma_channel_info *channel)
{
	...
	if (channel->nents && channel->dma_type != PCIE_DMA_PINNED_MEM &&
		...
		if (channel->p2p_sgl) {
			dma_unmap_sg(&pcie_set->pdev->dev, channel->p2p_sgl,
		...
		struct dma_channel_info *channel, int phy)
{
	...
	for (i = 0; i < pcie_set->dma_set.dma_fetch_buff; i++) {
		...
		fetch_num = i;
		if (__sync_bool_compare_and_swap(&pcie_set->dma_set.running_channels[phy][fetch_num],
		...
		running_phy_num = cn_pci_dma_phy_busy_num(pcie_set, phy);
		if (channel->task->count <= pcie_set->dma_set.dma_bypass_custom_size &&
			!channel->task->cfg.phy_mode &&
			running_phy_num <= (pcie_set->dma_set.dma_fetch_buff / 2 + 1)) {
			if (pcie_set->bus_set->core->board_info.platform == MLU_PLAT_ASIC)
			...
			struct cn_pcie_set *pcie_set)
{
	...
	cn_dev_pcie_info(pcie_set, "time out task:%lx", (unsigned long)task);
	cn_dev_pcie_info(pcie_set, "direction:%d count:%lx",
		task->transfer->direction, task->count);

	cn_dev_pcie_info(pcie_set, "task ready fifo len: %d",
			kfifo_len(&task->ready_fifo));

	cn_dev_pcie_info(pcie_set, "task finish fifo len: %d",
			kfifo_len(&task->finish_fifo));

	for (i = 0; i < pcie_set->dma_set.max_phy_channel; i++) {
		for (j = 0; j < pcie_set->dma_set.dma_fetch_buff; j++) {
			channel = (struct dma_channel_info *)pcie_set->dma_set.running_channels[i][j];
			if (channel && channel->task == task) {
				...
				cn_pci_print_channel_info(channel, pcie_set);
			}
		}
	}
}
...
loop:
	for (phy = 0; phy < pcie_set->dma_set.max_phy_channel; phy++) {
		for (command_id = 0; command_id < pcie_set->dma_set.dma_fetch_buff; command_id++) {
			channel = (struct dma_channel_info *)pcie_set->dma_set.running_channels[phy][command_id];
...
static size_t cn_pci_transfer_len(struct pcie_dma_task *task, size_t remain)
{
	...
	if ((task->count > pcie_set->dma_set.dma_buffer_size * 2) ||
			(remain <= 64 * 1024))
		return min(remain, (size_t)pcie_set->dma_set.dma_buffer_size);

	if ((remain < 128 * 1024))
		div = 2;

	len = task->count / div;
	len = min(len, pcie_set->dma_set.dma_buffer_size);
	len = min(len, remain);
	...
	if (task->p2p_linear_remap) {
		...
		return min_t(size_t, remain, (size_t)pcie_set->dma_set.dma_buffer_size);
	}
#endif
}
...
static unsigned long cn_pci_small_packet_write(struct pcie_dma_task *task)
{
	...
	/* h2d task number greater than 2 will use not small packet optimization,
	 * because bar write influence the stability of dma bandwidth.
	 */
	if (task->pcie_set->dma_set.task_sem_h2d.count < (DMA_TASK_MAX / 2 - 1))
		return 0;

	if ((direction == DMA_H2D) &&
			task->transfer->size <= task->pcie_set->dma_set.dma_bypass_custom_size &&
	...
	if ((direction == DMA_H2D) &&
			task->transfer->size <= task->pcie_set->dma_set.dma_bypass_pinned_size &&
...
static unsigned long cn_pci_small_packet_read(struct pcie_dma_task *task)
{
	...
	if ((direction == DMA_D2H) &&
			task->transfer->size <= task->pcie_set->dma_set.d2h_bypass_custom_size) {
...
static int cn_pci_finish_fifo_complete_out(struct cn_pcie_set *pcie_set, struct pcie_dma_task *task)
{
	...
	while (!kfifo_is_empty(&task->finish_fifo)) {
		...
		if (channel->status == CHANNEL_COMPLETED) {
			...
			cn_pci_channel_dma_end(channel);
			cn_dev_pcie_info(pcie_set, "mlu500s unsupport soft retry");
			return -1;
		}
	}
...
static size_t cn_pci_dma_transfer(struct pcie_dma_task *task)
{
	...
	if (pcie_set->state == PCIE_STATE_SUSPEND) {
		do {
			__sync_fetch_and_add(&pcie_set->dma_set.task_suspend_num, 1);
			ret = wait_event_interruptible(pcie_set->dma_set.task_suspend_wq,
				pcie_set->state != PCIE_STATE_SUSPEND);
			__sync_fetch_and_sub(&pcie_set->dma_set.task_suspend_num, 1);
	...
	while (1) {
		...
		if (remain > 0) {
			...
			if (channel->dma_type != PCIE_DMA_MEMSET)
				cpu_addr += len;
			ram_addr += len;
			remain -= len;
...
retry:
			if (!task->spkg_polling_flag) {
				...
				if (ret < 0) {
					...
					if (jiffies_to_msecs(pcie_set->dma_set.dma_timeout - ret) > 5000)
						cn_dev_pcie_info(pcie_set, "system busy time:%d",
							jiffies_to_msecs(pcie_set->dma_set.dma_timeout - ret));
				}
			} else {
				prt_start = get_jiffies_64();
polling_retry:
...
static int cn_pci_shared_desc_shm_init(struct cn_pcie_set *pcie_set)
{
	...
	ret = cn_device_share_mem_alloc(0, &host_kva, &dev_va,
			pcie_set->dma_set.shared_desc_total_size,
	...
	pcie_set->dma_set.shared_desc_dev_va = dev_va;
	pcie_set->dma_set.shared_desc_host_kva = host_kva;
...
static void cn_pci_shared_desc_shm_release(struct cn_pcie_set *pcie_set)
{
	...
	host_kva = pcie_set->dma_set.shared_desc_host_kva;
	dev_va = pcie_set->dma_set.shared_desc_dev_va;
		...
		struct dma_channel_info *channel, int id)
{
	...
	n = (pcie_set->dma_set.dma_buffer_size / PAGE_SIZE) + 1; /* 257 */
...
static int cn_pci_shared_dma_desc_init(struct cn_pcie_set *pcie_set)
{
	...
	pcie_set->dma_set.shared_channel_list = cn_kzalloc(
			sizeof(*pcie_set->dma_set.shared_channel_list) *
			pcie_set->dma_set.shared_channel_cnt,
			GFP_KERNEL);
	if (pcie_set->dma_set.shared_channel_list == NULL)
		return -1;
	...
	for (i = 0; i < pcie_set->dma_set.shared_channel_cnt;  i++) {
		...
		channel->desc_size = pcie_set->dma_set.shared_channel_desc_cnt *
		...
		pcie_set->dma_set.shared_channel_list[i] = channel;
...
static void cn_pci_shared_dma_desc_release(struct cn_pcie_set *pcie_set)
{
	...
	for (i = 0; i < pcie_set->dma_set.shared_channel_cnt; i++) {
		...
		if (channel) {
			cn_pci_shared_dma_channel_soft_release(channel);
			cn_kfree(channel);
			pcie_set->dma_set.shared_channel_list[i] = NULL;
		}
	}

	cn_kfree(pcie_set->dma_set.shared_channel_list);
}
...
static int cn_pci_priv_desc_order_table_init(struct cn_pcie_set *pcie_set)
{
	...
	for (index = 0; index < DMA_TASK_MAX; index++) {
		...
		task->priv_order_table = cn_kzalloc(
			sizeof(*task->priv_order_table) * pcie_set->dma_set.max_desc_order, GFP_KERNEL);
...
static void cn_pci_priv_desc_order_table_release(struct cn_pcie_set *pcie_set)
{
	...
	if (!pcie_set->dma_set.task_table)
		return;
...
static int cn_pci_priv_desc_shm_init(struct cn_pcie_set *pcie_set)
{
	...
	ret = cn_device_share_mem_alloc(0, &host_kva, &dev_va,
			pcie_set->dma_set.priv_desc_total_size,
			pcie_set->bus_set->core);
	if (ret) {
	...
	pcie_set->dma_set.priv_desc_dev_va = dev_va;
	pcie_set->dma_set.priv_desc_host_kva = host_kva;
...
static void cn_pci_priv_desc_shm_release(struct cn_pcie_set *pcie_set)
{
	...
	host_kva = pcie_set->dma_set.priv_desc_host_kva;
	dev_va = pcie_set->dma_set.priv_desc_dev_va;
...
static int cn_pci_priv_dma_desc_init(struct cn_pcie_set *pcie_set)
{
	...
	for (index = 0; index < DMA_TASK_MAX; index++) {
		task = pcie_set->dma_set.task_table[index];
		total = pcie_set->dma_set.priv_desc_total_size / DMA_TASK_MAX;
		for (i = (pcie_set->dma_set.max_desc_order - 1); i >= 0; i--) {
...
static int cn_pci_check_priv_dma_desc(struct cn_pcie_set *pcie_set)
{
	...
	for (index = 0; index < DMA_TASK_MAX; index++) {
		...
		if (size > pcie_set->dma_set.priv_desc_total_size) {
...
static int cn_pci_dma_priv_channel_mem_init(struct cn_pcie_set *pcie_set)
{
	...
	cnt = pcie_set->dma_set.priv_desc_total_size / pcie_set->per_desc_size
		/ DMA_TASK_MAX / 2 / (1 << (pcie_set->dma_set.max_desc_order - 1));
	if (!cnt) {
		cn_dev_pcie_info(pcie_set, "don't init dma priv channel");
		cn_dev_pcie_info(pcie_set, "priv_desc_total_size:%#lx per_desc_size:%d",
			pcie_set->dma_set.priv_desc_total_size, pcie_set->per_desc_size);
		cn_dev_pcie_info(pcie_set, " DMA_TASK_MAX:%d max_desc_order:%d",
			DMA_TASK_MAX, (pcie_set->dma_set.max_desc_order - 1));
		return 0;
	}
...
static int cn_pci_dma_channel_mem_init(struct cn_pcie_set *pcie_set)
{
	 ...
	 */
	max_pages = roundup_pow_of_two((pcie_set->dma_set.dma_buffer_size / PAGE_SIZE) + 1);
	max_order = order_base_2(max_pages);
	pcie_set->dma_set.max_desc_order = max_order;

	/* Hardware Limit: desc addr 64 Bytes align */
	pcie_set->dma_set.shared_channel_desc_cnt = pcie_set->dma_set.dma_buffer_size / PAGE_SIZE + 2;
	pcie_set->dma_set.shared_channel_cnt = pcie_set->dma_set.shared_desc_total_size /
			pcie_set->per_desc_size / pcie_set->dma_set.shared_channel_desc_cnt;
...
cn_pci_get_dma_idle_task(struct cn_pcie_set *pcie_set, DMA_DIR_TYPE direction)
{
	 ...
	 */
	if (direction == DMA_H2D) {
		ret = down_killable(&pcie_set->dma_set.task_sem_h2d);
	} else {
		ret = down_killable(&pcie_set->dma_set.task_sem_d2h);
	}
	if (ret) {
...
retry:
	for (i = 0; i < DMA_TASK_MAX; i++) {
		task = pcie_set->dma_set.task_table[i];
		if (direction == DMA_H2D) {
			if (i % 2)
				continue;
		} else {
		...
		struct pcie_dma_task *task)
{
	...
	if (direction == DMA_H2D) {
		up(&pcie_set->dma_set.task_sem_h2d);
	} else {
		up(&pcie_set->dma_set.task_sem_d2h);
	}
}
		...
		struct pcie_dma_task *task)
{
	...
	share_cnt = ((pcie_set->dma_set.shared_desc_total_size / 2 / pcie_set->per_desc_size)
			>> max_order) * (max_order + 1);

	priv_cnt = ((pcie_set->dma_set.priv_desc_total_size / 2 / pcie_set->per_desc_size
	 ...
	 */
	desc_num = roundup_pow_of_two((pcie_set->dma_set.dma_buffer_size / PAGE_SIZE) + 1);
...
static int cn_pci_dma_task_init(struct cn_pcie_set *pcie_set)
{
	...
	pcie_set->dma_set.task_table = cn_kzalloc(
			sizeof(task) * DMA_TASK_MAX, GFP_KERNEL);
	if (pcie_set->dma_set.task_table == NULL)
		goto exit;
	...
	for (i = 0; i < DMA_TASK_MAX; i++) {
		...
		if (ret)
			goto exit;
		pcie_set->dma_set.task_table[i] = task;
	}

	sema_init(&pcie_set->dma_set.task_sem_h2d, DMA_TASK_MAX / 2);
	sema_init(&pcie_set->dma_set.task_sem_d2h, DMA_TASK_MAX / 2);
...
static void cn_pci_dma_task_release(struct cn_pcie_set *pcie_set)
{
	...
	cn_kfree(pcie_set->dma_set.task_table);
	pcie_set->dma_set.task_table = NULL;
}
...
static int cn_pci_dma_async_free_init(struct cn_pcie_set *pcie_set)
{
	...
	if (!pcie_set->cfg.af_enable)
		return 0;

	/* num_order[0] maybe equal to 2 * num_order[max_order] */
	share_cnt = ((pcie_set->dma_set.shared_desc_total_size / 2 / pcie_set->per_desc_size)
			>> pcie_set->dma_set.max_desc_order) * (pcie_set->dma_set.max_desc_order + 1);
	priv_cnt = ((pcie_set->dma_set.priv_desc_total_size / 2 / pcie_set->per_desc_size)
			>> (pcie_set->dma_set.max_desc_order - 1)) * pcie_set->dma_set.max_desc_order;
	cn_dev_pcie_debug(pcie_set,
			"share_cnt[%lld], priv_cnt[%lld]", share_cnt, priv_cnt);
...
static int cn_pci_dma_af_ctrl(void *pcie_priv, unsigned int enable)
{
	...
	if (enable) {
		pcie_set->cfg.af_enable = 1;
		cn_dev_pcie_info(pcie_set, "dma async free enable");
	} else {
		pcie_set->cfg.af_enable = 0;
...
static void cn_pci_dma_phy_channel_release(struct cn_pcie_set *pcie_set)
{
	...
	if (!pcie_set->dma_set.running_channels)
		return;

	for (i = 0; i < pcie_set->dma_set.max_phy_channel; i++) {
		if (pcie_set->dma_set.running_channels[i]) {
			cn_kfree(pcie_set->dma_set.running_channels[i]);
			pcie_set->dma_set.running_channels[i] = NULL;
		}
	}

	if (pcie_set->dma_set.running_channels) {
		cn_kfree(pcie_set->dma_set.running_channels);
		pcie_set->dma_set.running_channels = NULL;
	}
}
...
static int cn_pci_dma_phy_channel_init(struct cn_pcie_set *pcie_set)
{
	...
	pcie_set->dma_set.running_channels = cn_kzalloc((pcie_set->dma_set.max_phy_channel) *
	...
	bitmap_size = BITS_TO_LONGS(pcie_set->dma_set.dma_fetch_buff) * sizeof(long);
	for (i = 0; i < pcie_set->dma_set.max_phy_channel; i++) {
		pcie_set->dma_set.running_channels[i] = cn_kzalloc(pcie_set->dma_set.dma_fetch_buff *
					(sizeof(unsigned long)), GFP_KERNEL);
		if (!pcie_set->dma_set.running_channels[i]) {
...
static int cn_pci_dma_sync_init(struct cn_pcie_set *pcie_set)
{
	...
	for (i = 0; i < 8; i++) {
		spin_lock_init(&pcie_set->dma_set.fetch_lock[i]);
	}
...
static int cn_pci_dma_suspend(void *pcie_priv)
{
	...
	while (pcie_set->dma_set.task_num != pcie_set->dma_set.task_suspend_num) {
		schedule();
		udelay(1);
	}

	cn_dev_pcie_info(pcie_set, "task_num:%d task_suspend_num:%d",
		pcie_set->dma_set.task_num, pcie_set->dma_set.task_suspend_num);

	...
	if (async_task->dma_type == PCIE_DMA_P2P) {
		if (async_task->async_info->desc_device_va) {
			if (async_task->pcie_set->cfg.p2p_mode == P2P_PULL_MODE) {
				cn_mem_linear_unmap(async_task->peer.src_minfo,
					async_task->sg_tb);
			} else {
		...
		cn_async_address_kref_put((u64)async_task->peer.src_minfo, async_task->peer.src_addr,
				async_task->peer.size);
		cn_async_address_kref_put((u64)async_task->peer.dst_minfo, async_task->peer.dst_addr,
				async_task->peer.size);
...
static void cn_pci_async_dma_task_release(struct cn_pcie_set *pcie_set)
{
	...
	/* async task bitmap */
	cn_kfree(pcie_set->async_set.async_task_bitmap);

	/* async info table */
	if (!pcie_set->async_set.async_info_table)
		return;
	for (i = 0; i < pcie_set->async_set.async_static_task_num; i++) {
		async_info = pcie_set->async_set.async_info_table[i];
		cn_kfree(async_info);
	}
	cn_kfree(pcie_set->async_set.async_info_table);
	pcie_set->async_set.async_info_table = NULL;

	/* async task table */
	if (!pcie_set->async_set.async_task_table)
		return;
	for (i = 0; i < pcie_set->async_set.async_static_task_num; i++) {
		task = pcie_set->async_set.async_task_table[i];
		cn_kfree(task);
	}
	cn_kfree(pcie_set->async_set.async_task_table);
	pcie_set->async_set.async_task_table = NULL;

	if (pcie_set->cfg.arm_trigger_enable) {
		cn_vfree(pcie_set->async_set.async_desc_buf);
		cn_vfree(pcie_set->async_set.async_chunk);
		cn_vfree(pcie_set->async_set.async_sg_list);
		cn_vfree(pcie_set->async_set.async_pp_pages);
		if (pcie_set->async_set.async_desc_host_kva &&
			pcie_set->async_set.async_desc_dev_va) {
			cn_device_share_mem_free(0, pcie_set->async_set.async_desc_host_kva,
					pcie_set->async_set.async_desc_dev_va,
					pcie_set->bus_set->core);
		}
		cn_kfree(pcie_set->async_set.async_desc_bitmap);
	}
}
...
static int cn_pci_async_dma_task_init(struct cn_pcie_set *pcie_set)
{
	...
	/* desc buf */
	if (pcie_set->cfg.arm_trigger_enable) {
		bitmap_size = BITS_TO_LONGS(pcie_set->async_set.async_desc_num) *
					sizeof(long);
		pcie_set->async_set.async_desc_bitmap = cn_kzalloc(bitmap_size, GFP_KERNEL);
		if (pcie_set->async_set.async_desc_bitmap == NULL) {
			cn_dev_pcie_err(pcie_set, "async desc bitmap alloc fail");
			goto exit;
		}
		spin_lock_init(&pcie_set->async_set.async_desc_lock);

		ret = cn_device_share_mem_alloc(0, &pcie_set->async_set.async_desc_host_kva,
				&pcie_set->async_set.async_desc_dev_va,
				pcie_set->async_set.async_desc_size,
				pcie_set->bus_set->core);
		if (ret) {
		...
		pcie_set->async_set.async_pp_pages = cn_vzalloc(sizeof(struct page *) *
					pcie_set->async_set.async_desc_size /
					pcie_set->per_desc_size);
		if (pcie_set->async_set.async_pp_pages == NULL)
			goto exit;

		pcie_set->async_set.async_sg_list = cn_vzalloc(sizeof(struct scatterlist) *
					pcie_set->async_set.async_desc_size /
					pcie_set->per_desc_size);
		if (pcie_set->async_set.async_sg_list == NULL)
			goto exit;

		pcie_set->async_set.async_chunk = cn_vzalloc(sizeof(int) *
					pcie_set->async_set.async_desc_size /
					pcie_set->per_desc_size);
		if (pcie_set->async_set.async_chunk == NULL)
			goto exit;

		pcie_set->async_set.async_desc_buf = cn_vzalloc(pcie_set->async_set.async_desc_size);
		if (pcie_set->async_set.async_desc_buf == NULL)
			goto exit;
	}

	/* async task table */
	pcie_set->async_set.async_task_table = cn_kzalloc(
			sizeof(task) * pcie_set->async_set.async_static_task_num,
			GFP_KERNEL);
	if (pcie_set->async_set.async_task_table == NULL)
		goto exit;
	for (i = 0; i < pcie_set->async_set.async_static_task_num; i++) {
		...
		if (task == NULL)
			goto exit;
		task->id = i;
		pcie_set->async_set.async_task_table[i] = task;
	}

	/* async info table */
	pcie_set->async_set.async_info_table = cn_kzalloc(
			sizeof(async_info) * pcie_set->async_set.async_static_task_num,
			GFP_KERNEL);
	if (pcie_set->async_set.async_info_table == NULL)
		goto exit;
	for (i = 0; i < pcie_set->async_set.async_static_task_num; i++) {
		async_info = cn_kzalloc(sizeof(*async_info), GFP_KERNEL);
		if (async_info == NULL)
			goto exit;
		pcie_set->async_set.async_info_table[i] = async_info;
	}

	/* async task bitmap */
	bitmap_size = BITS_TO_LONGS(pcie_set->async_set.async_static_task_num) *
				sizeof(long);
	pcie_set->async_set.async_task_bitmap = cn_kzalloc(bitmap_size, GFP_KERNEL);
	if (pcie_set->async_set.async_task_bitmap == NULL) {
		cn_dev_pcie_err(pcie_set, "async task bitmap alloc fail");
		goto exit;
	}
	spin_lock_init(&pcie_set->async_set.async_task_lock);
...
retry:
	spin_lock(&pcie_set->async_set.async_desc_lock);
	bit_index = bitmap_find_next_zero_area(pcie_set->async_set.async_desc_bitmap,
			pcie_set->async_set.async_desc_num, 0,
			desc_num, 0);
	if (bit_index >= pcie_set->async_set.async_desc_num) {
		spin_unlock(&pcie_set->async_set.async_desc_lock);

		if (desc_num > pcie_set->async_set.async_max_desc_num)
			return -1;
	...
	bitmap_set(pcie_set->async_set.async_desc_bitmap, bit_index, desc_num);
	spin_unlock(&pcie_set->async_set.async_desc_lock);

	task->bit_index = bit_index;
	task->desc_num = desc_num;
	task->dev_desc_addr = pcie_set->async_set.async_desc_dev_va +
		bit_index * pcie_set->per_desc_size;
	task->host_desc_addr = pcie_set->async_set.async_desc_host_kva +
		bit_index * pcie_set->per_desc_size;
	task->desc_buf = (void *)((u64)pcie_set->async_set.async_desc_buf +
			bit_index * pcie_set->per_desc_size);
	task->pp_pages = (struct page **)((u64)pcie_set->async_set.async_pp_pages +
			bit_index * sizeof(struct page *));
	task->chunk = (int *)((u64)pcie_set->async_set.async_chunk +
			bit_index * sizeof(int));
	task->sg_list = (struct scatterlist *)((u64)pcie_set->async_set.async_sg_list +
...
static void cn_pci_async_put_task_desc(struct async_task *task)
{
	...
	if (task->desc_num) {
		spin_lock(&pcie_set->async_set.async_desc_lock);
		bitmap_clear(pcie_set->async_set.async_desc_bitmap,
		...
		struct async_task **task, struct dma_async_info_s **info)
{
	...
	spin_lock(&pcie_set->async_set.async_task_lock);
	index = find_first_zero_bit(pcie_set->async_set.async_task_bitmap,
			pcie_set->async_set.async_static_task_num);
	if (index >= pcie_set->async_set.async_static_task_num) {
		spin_unlock(&pcie_set->async_set.async_task_lock);
		goto dynamic_alloc;
	}
	set_bit(index, pcie_set->async_set.async_task_bitmap);
	spin_unlock(&pcie_set->async_set.async_task_lock);

	async_task = pcie_set->async_set.async_task_table[index];
	async_info = pcie_set->async_set.async_info_table[index];
		...
		struct async_task *task, struct dma_async_info_s *async_info)
{
	...
	if (task->dynamic_alloc_flag) {
		...
		spin_lock(&pcie_set->async_set.async_task_lock);
		clear_bit(task->id, pcie_set->async_set.async_task_bitmap);
		spin_unlock(&pcie_set->async_set.async_task_lock);
	}
}
	...
	if (task->nents) {
		...
		if ((task->dma_type == PCIE_DMA_P2P) &&
			...
			if (pcie_set->cfg.p2p_mode == P2P_PULL_MODE) {
				dma_unmap_sg(&task->pcie_set_dst->pdev->dev, task->sg_list,
					task->nents, DMA_BIDIRECTIONAL);
			} else {
				dma_unmap_sg(&task->pcie_set_src->pdev->dev, task->sg_list,
					task->nents, DMA_BIDIRECTIONAL);
			}
		}
	}
...
static int cn_pci_dma_abort(u64 tags, u64 index, void *pcie_priv)
{
	...
	hash_for_each_possible_safe(pcie_set->async_set.async_task_htable,
...
static int cn_pci_get_desc_num(struct async_task *task)
{
	...
	case PCIE_DMA_PINNED_MEM:
		...
		break;
	case PCIE_DMA_P2P:
		desc_num = cn_pci_get_linear_p2p_desc_num(pcie_set, task->sg_tb) + 2;
		break;
	default:
...
{
	struct cn_pcie_set *pcie_set = task->pcie_set_stream;
	int nents;
	...
	case PCIE_DMA_P2P:
		if (pcie_set->cfg.p2p_mode == P2P_PULL_MODE) {
			nents = dma_map_sg(&task->pcie_set_dst->pdev->dev, task->sg_list,
					task->nents, DMA_BIDIRECTIONAL);
		} else {
			nents = dma_map_sg(&task->pcie_set_src->pdev->dev, task->sg_list,
					task->nents, DMA_BIDIRECTIONAL);
		}
		if (!nents) {
		...
		struct dma_async_info_s **pinfo, void *pcie_priv)
{
	...
	*pinfo = async_info;
	...
	if (!pcie_set_stream->cfg.arm_trigger_enable) {
...
host_trigger_task:
	mutex_lock(&pcie_set_stream->async_set.async_task_hash_lock);
	hash_add(pcie_set_stream->async_set.async_task_htable, &task->hlist,
				async_info->tags + async_info->index);
	mutex_unlock(&pcie_set_stream->async_set.async_task_hash_lock);

	if (async_info->desc_device_va) {
		__sync_fetch_and_add(&pcie_set_stream->async_set.arm_trigger_dma_cnt, 1);
	} else {
		__sync_fetch_and_add(&pcie_set_stream->async_set.host_trigger_dma_cnt, 1);
	}
		...
		struct dma_async_info_s **pinfo, void *pcie_priv)
{
	...
	if (pcie_set_dst->cfg.p2p_mode == P2P_PULL_MODE) {
		pcie_set_master = pcie_set_dst;
		pcie_set_slv = pcie_set_src;
		slv_minfo = t->src_minfo;
		slv_addr = t->src_addr;
	} else {
		pcie_set_master = pcie_set_src;
		pcie_set_slv = pcie_set_dst;
		slv_minfo = t->dst_minfo;
		slv_addr = t->dst_addr;
	}

	ret = cn_pci_get_async_dma_idle_task(pcie_set_master, &task, &async_info);
	if (ret) {
		cn_dev_pcie_err(pcie_set_master, "get idle async task fail");
		return -1;
	}
	...
	*pinfo = async_info;
	...
	task->pcie_set = pcie_set_master;
	task->pcie_set_src = pcie_set_src;
	...
	task->index = async_info->index;
	task->user = t->user;
	task->clockid = get_host_timestamp_clockid(t->user, pcie_set_master->bus_set->core);
	INIT_WORK(&task->trigger_work, cn_pci_async_message_work);
	...
	if (pcie_set_stream != pcie_set_master) {
		async_info->reason = ASYNC_REASON_STREAM_INVALID;
		goto host_trigger_task;
	}
	...
	/* device trigger */
	task->sg_tb = cn_mem_linear_remap(slv_minfo, slv_addr, t->size);
	...
	desc_num = cn_pci_get_desc_num(task);
	if (cn_pci_async_get_task_desc(task, desc_num)) {
		cn_mem_linear_unmap(slv_minfo, task->sg_tb);
		async_info->reason = ASYNC_REASON_DESC_NOT_ENOUGH;
		goto host_trigger_task;
	}
	...
	if (pcie_set_stream->cfg.p2p_mode == P2P_PUSH_MODE) {
		if (pcie_set_stream->ops->async_dma_fill_p2p_pull_desc(task)) {
			cn_mem_linear_unmap(slv_minfo, task->sg_tb);
			cn_pci_async_dma_task_end(task);
			cn_pci_async_put_task_desc(task);
			cn_dev_pcie_err(pcie_set_stream, "async dma fill p2p pull desc fail");
			async_info->reason = ASYNC_REASON_P2P_FILL_PULL_DESC_ERR;
			goto host_trigger_task;
		}
	}
...
host_trigger_task:
	...
	if (async_info->desc_device_va) {
		__sync_fetch_and_add(&pcie_set_stream->async_set.arm_trigger_p2p_cnt, 1);
	} else {
		__sync_fetch_and_add(&pcie_set_stream->async_set.host_trigger_p2p_cnt, 1);
	}
		...
		struct dma_async_info_s **pinfo, void *pcie_priv)
{
	...
	if (t->direction == MEMSET_D8) {
		...
		if (pcie_set->cfg.arm_trigger_enable &&
			async_info->total_size <= 0x1000000 &&
			pcie_set == pcie_set_stream)
			async_info->desc_device_va = 1;
	...
	async_info->reason = ASYNC_REASON_DEVICE_MEMSET;
	*pinfo = async_info;
	...
	mutex_lock(&pcie_set_stream->async_set.async_task_hash_lock);
	hash_add(pcie_set_stream->async_set.async_task_htable, &async_task->hlist,
				async_info->tags + async_info->index);
	mutex_unlock(&pcie_set_stream->async_set.async_task_hash_lock);
...
static int cn_pci_get_async_htable(void *pcie_priv)
{
	...
	struct hlist_node *tmp;
	int i = 0;
	int hash_size = HASH_SIZE(pcie_set->async_set.async_task_htable);
	u32 used_cnt;
	int flag = 0;
	...
	for (i = 0; i < hash_size; i++) {
		...
		hlist_for_each_entry_safe(async_task, tmp,
				&pcie_set->async_set.async_task_htable[i], hlist) {
			used_cnt++;
		}
		if (used_cnt) {
...
static int cn_pci_dma_async_init(struct cn_pcie_set *pcie_set)
{
	...
	mutex_init(&pcie_set->async_set.async_task_hash_lock);

	if (cn_pci_async_dma_task_init(pcie_set)) {
		return -1;
	}

	hash_init(pcie_set->async_set.async_task_htable);

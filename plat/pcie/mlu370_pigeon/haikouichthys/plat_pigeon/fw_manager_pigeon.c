...
#if 0 /*No fmt handle for call printk*/
...
#include "cndrv_bus.h"
#include "../../cndrv_pci.h"
#include "pigeon_zebu_ddr_data.h"

#include "cndrv_pci_pigeon.h"

#define PIGEON_FPGA_DDR_INIT 0
#define PIGEON_LPDDR4_1DIE_DDR_INIT 0
#define PIGEON_LPDDR5_1DIE_DDR_INIT 0

#if ((PIGEON_FPGA_DDR_INIT + PIGEON_LPDDR4_1DIE_DDR_INIT + PIGEON_LPDDR5_1DIE_DDR_INIT) > 1)
#error "Only can do one plat for Temp DDR Init"
#endif
...
struct reg_dict_s {
	...
	unsigned long cnt;
	unsigned long base;
	struct reg_list_s *reg_list;
};

struct reg_list_s top_cfg_sw_ctl_list[] = {
	{0xA0, 0xFFFFFFFF},
};
struct reg_list_s llcsys_reset_sw_list[] = {
	{0x44, 0x10},
	{0x44, 0x13},
	{0x44, 0x1F},
};
struct reg_list_s llc_slv_init_list[] = {
	{0xD8, 0x1F},
};
struct reg_list_s fpga_1die_memsys_init_list[] = {
	{0x4100, 0x1FFF},
	{0x4104, 0x3FF03FFF},
	{0x2008, 0x20},
	{0x200C, 0x00},
};
struct reg_dict_s fpga_memsys_reg_dict[] = {
	{"top_cfg_sw_ctl", 1, 0x1800, top_cfg_sw_ctl_list},
	{"llcsys_reset_sw", 3, 0x680000, llcsys_reset_sw_list},
	{"llc_slv_init", 1, 0x681000, llc_slv_init_list},
	{"fpga_1die_memsys_int", 4, 0x700000, fpga_1die_memsys_init_list},
	{NULL, 0, 0, NULL},
};
...
static void cn_pci_reg_write32(struct cn_pcie_set *pcie_set, unsigned long offset, u32 val)
{
	if (pcie_set->ops->reg_write32) {
		cn_dev_info("fwmg call pcie_set.ops.reg_write32");
		pcie_set->ops->reg_write32(offset, val, pcie_set);
	} else {
		cn_dev_info("fwmg Direct iowrite32");
...
static u32 cn_pci_reg_read32(struct cn_pcie_set *pcie_set, unsigned long offset)
{
	if (pcie_set->ops->reg_read32) {
		cn_dev_info("fwmg call pcie_set.ops.reg_read32");
		return pcie_set->ops->reg_read32(offset, pcie_set);
	} else {
		cn_dev_info("fwmg Direct ioread32");
		return ioread32(pcie_set->reg_virt_base + offset);
	}
}

static void ddr_reg_wt32(unsigned long offset, u32 value, void *pcie_set)
{
	cn_pci_reg_write32(pcie_set, offset, value);
}

static u32 ddr_reg_rd32(unsigned long offset, void *pcie_set)
{
	uint32_t val = 0;
...
static void write_lpddr_mr(uint64_t base_addr, uint32_t mr, uint32_t rank,
...
static void tranverse_init_reg_dict(struct cn_pcie_set *pcie_set, struct reg_dict_s *reg_dict)
{
	unsigned int i = 0;
	unsigned int j = 0;
	unsigned long addr;
	...
	while (reg_dict[i].module_name) {
		cn_dev_info("Init register of module - %s", reg_dict[i].module_name);
		for (j = 0; j < reg_dict[i].cnt; j++) {
			addr = reg_dict[i].base + reg_dict[i].reg_list[j].addr;
			data = reg_dict[i].reg_list[j].data;
			ddr_reg_wt32(addr, data, pcie_set);
			ddr_reg_rd32(addr, pcie_set);
			cn_dev_info("write [%#lx] to [%#lx]", data, addr);
		}
		i++;
	}
}
...
/*
	DDR4x
		pigeon_only_one_die_init_full_lpddr4x
			pigeon_lpddr4_memsys_init
				config_lpddr4
					write_lpddr_mr
	DDR5
		pigeon_only_one_die_init_full_lpddr5
			pigeon_lpddr5_memsys_init
				config_lpddr5
					write_lpddr_mr

	Temp FPGA/ZEBU
		pigeon_fpga_memsys_init
 */
static void config_lpddr4(uint64_t base_addr, struct cn_pcie_set *pcie_set)
{
	uint32_t data;
	uint64_t addr;
...
static void config_lpddr5(uint64_t base_addr, struct cn_pcie_set *pcie_set)
{
	uint32_t data;
	uint64_t addr;
...
void pigeon_fpga_memsys_init(struct cn_pcie_set *pcie_set)
{
	/*1. top config software control*/
	/*2. llc reset*/
	/*3. 1-die memsys init*/
	tranverse_init_reg_dict(pcie_set, fpga_memsys_reg_dict);
}

static void pigeon_lpddr4_memsys_init(struct cn_pcie_set *pcie_set)
{
	uint32_t val;
 ...
 *
	...
	NoC ---
		...
		---- mdcsys --- |
				----- umc1/chn1 (DDR4/2GB)
 */
void pigeon_only_one_die_init_full_lpddr4x(struct cn_pcie_set *pcie_set)
{
	ddr_reg_wt32(0xD00100, 0x1, pcie_set); //mdc_model_sel_reg: 0x0 work on D2D; 0x1 work on Mem
	pigeon_lpddr4_memsys_init(pcie_set);
...
static void pigeon_lpddr5_memsys_init(struct cn_pcie_set *pcie_set)
{
	uint32_t val;
 ...
 *
	...
	NoC ---
		...
		---- mdcsys --- |
				----- umc1/chn1 (DDR5/4GB)
 */
void pigeon_only_one_die_init_full_lpddr5(struct cn_pcie_set *pcie_set)
{
	ddr_reg_wt32(0xD00100, 0x1, pcie_set); //ddrc_apd_resetn
	pigeon_lpddr5_memsys_init(pcie_set);
...
{
#if PIGEON_LPDDR4_1DIE_DDR_INIT
	struct cn_pcie_set *pcie_set = (struct cn_pcie_set *)pcie;

	cn_dev_info("To do 1Die-DDR4 init by Driver");
	pigeon_only_one_die_init_full_lpddr4x(pcie_set);
#elif PIGEON_LPDDR5_1DIE_DDR_INIT
	struct cn_pcie_set *pcie_set = (struct cn_pcie_set *)pcie;

	cn_dev_info("To do 1Die-DDR5 init by Driver");
	pigeon_only_one_die_init_full_lpddr5(pcie_set);
#elif PIGEON_FPGA_DDR_INIT
	struct cn_pcie_set *pcie_set = (struct cn_pcie_set *)pcie;

	cn_dev_info("To do FPGA-DDR init by Driver");
	pigeon_fpga_memsys_init(pcie_set);
#else
	cn_dev_info("No selection to do DDR init by Driver");
#endif
}

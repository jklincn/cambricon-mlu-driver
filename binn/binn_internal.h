...
#ifndef BINN_INTERNAL_H
#define BINN_INTERNAL_H
...
struct binn {
	/* this struct header holds the magic number (BINN_MAGIC) that identifies */
	/*  this memory block as a binn structure */
	int    header;
	BOOL   allocated;  /* the struct can be allocated using malloc_fn() or can be on the stack */
	/* did it was create for writing? it can use the pbuf if not unified with ptr */
	BOOL   writable;
...
BOOL APIENTRY binn_create(struct binn *item, int type, int size, void *buffer);
...
/* free the binn structure but keeps the binn buffer allocated, */
/* returning a pointer to it. use the free function to release the buffer later */
...
static inline struct binn *binn_int8(signed char value)
{
	return binn_value(BINN_INT8, &value, 0, NULL);
}

static inline struct binn *binn_int16(short value)
{
	return binn_value(BINN_INT16, &value, 0, NULL);
}

static inline struct binn *binn_int32(int value)
{
	return binn_value(BINN_INT32, &value, 0, NULL);
}

static inline struct binn *binn_int64(int64 value)
{
	return binn_value(BINN_INT64, &value, 0, NULL);
}

static inline struct binn *binn_uint8(unsigned char value)
{
	return binn_value(BINN_UINT8, &value, 0, NULL);
}

static inline struct binn *binn_uint16(unsigned short value)
{
	return binn_value(BINN_UINT16, &value, 0, NULL);
}

static inline struct binn *binn_uint32(unsigned int value)
{
	return binn_value(BINN_UINT32, &value, 0, NULL);
}

static inline struct binn *binn_uint64(uint64 value)
{
	return binn_value(BINN_UINT64, &value, 0, NULL);
}

static inline struct binn *binn_float(float value)
{
	return binn_value(BINN_FLOAT, &value, 0, NULL);
}

static inline struct binn *binn_double(double value)
{
	return binn_value(BINN_DOUBLE, &value, 0, NULL);
}

static inline struct binn *binn_bool(BOOL value)
{
	return binn_value(BINN_BOOL, &value, 0, NULL);
}
static inline struct binn *binn_null(void)
{
	return binn_value(BINN_NULL, NULL, 0, NULL);
}

static inline struct binn *binn_string(char *str, binn_mem_free freefn)
{
	return binn_value(BINN_STRING, str, 0, freefn);
}

static inline struct binn *binn_blob(void *ptr, int size, binn_mem_free freefn)
{
...
BOOL   APIENTRY binn_load(void *data, struct binn *item);  /* on stack */
struct binn *APIENTRY binn_open(void *data);              /* allocated */
...
/* return a pointer to an allocated binn structure - must be released with the free() */
/* function or equivalent set in binn_set_alloc_functions() */
struct binn *APIENTRY binn_list_value(void *list, int pos);
struct binn *APIENTRY binn_map_value(void *map, int id);
struct binn *APIENTRY binn_object_value(void *obj, char *key);

/* read the value to a binn structure on the stack */
BOOL APIENTRY binn_list_get_value(void *list, int pos, struct binn *value);
BOOL APIENTRY binn_map_get_value(void *map, int id, struct binn *value);
BOOL APIENTRY binn_object_get_value(void *obj, char *key, struct binn *value);
...
/* on little-endian devices they are thread-safe only to return pointers to list, */
/*  map, object, blob and strings */
/* the returned pointer to 16, 32 and 64 bits values must be used only by */
...
/* on stack */
BOOL APIENTRY binn_map_get_pair(void *map, int pos, int *pid, struct binn *value);
/* must free the memory returned in the pkey */
BOOL APIENTRY binn_object_get_pair(void *obj, int pos, char *pkey, struct binn *value);
...
/* they are thread-safe on big-endian devices */
/* on little-endian devices they are thread-safe only to return pointers to */
/* list, map, object, blob and strings */
...
BOOL   APIENTRY binn_iter_init(struct binn_iter *iter, void *pbuf, int type);
...
/* the key must be declared as: char key[256]; */
...
/* the key must be declared as: char key[256]; */
BOOL APIENTRY binn_object_next(struct binn_iter *iter, char *pkey,
	struct binn *value);
...
#define binn_set_bool(item, value)   \
	do { (item)->type = BINN_BOOL; \
		(item)->vbool = value; \
		(item)->ptr = &((item)->vbool); \
	} \
	while (0)

#define binn_set_int(item, value)    \
	do { (item)->type = BINN_INT32; \
		(item)->vint32 = value; \
		(item)->ptr = &((item)->vint32); \
	} while (0)

#define binn_set_int64(item, value)  \
	do { (item)->type = BINN_INT64; \
		(item)->vint64 = value; \
		(item)->ptr = &((item)->vint64); \
	} while (0)

#define binn_set_uint(item, value)   \
	do { (item)->type = BINN_UINT32; \
		(item)->vuint32 = value; \
		(item)->ptr = &((item)->vuint32); \
	} while (0)

#define binn_set_uint64(item, value) \
	do { \
		(item)->type = BINN_UINT64; \
		(item)->vuint64 = value; \
		(item)->ptr = &((item)->vuint64); \
	} while (0)

#define binn_set_float(item, value)  \
	do { \
		(item)->type = BINN_FLOAT;  \
		(item)->vfloat  = value; \
		(item)->ptr = &((item)->vfloat); \
	} while (0)

#define binn_set_double(item, value) \
	do { \
		(item)->type = BINN_DOUBLE; \
		(item)->vdouble = value; \
		(item)->ptr = &((item)->vdouble); \
	} while (0)

BOOL APIENTRY binn_set_string(struct binn *item, char *str, binn_mem_free pfree);
BOOL APIENTRY binn_set_blob(struct binn *item, void *ptr, int size,
	binn_mem_free pfree);
...
static inline BOOL binn_list_add_int8(struct binn *list, signed char value) {
	return binn_list_add(list, BINN_INT8, &value, 0);
}

static inline BOOL binn_list_add_int16(struct binn *list, short value) {
	return binn_list_add(list, BINN_INT16, &value, 0);
}

static inline BOOL binn_list_add_int32(struct binn *list, int value) {
	return binn_list_add(list, BINN_INT32, &value, 0);
}

static inline BOOL binn_list_add_int64(struct binn *list, int64 value) {
	return binn_list_add(list, BINN_INT64, &value, 0);
}

static inline BOOL binn_list_add_uint8(struct binn *list, unsigned char value) {
	return binn_list_add(list, BINN_UINT8, &value, 0);
}

static inline BOOL binn_list_add_uint16(struct binn *list, unsigned short value) {
	return binn_list_add(list, BINN_UINT16, &value, 0);
}

static inline BOOL binn_list_add_uint32(struct binn *list, unsigned int value) {
	return binn_list_add(list, BINN_UINT32, &value, 0);
}

static inline BOOL binn_list_add_uint64(struct binn *list, uint64 value) {
	return binn_list_add(list, BINN_UINT64, &value, 0);
}

static inline BOOL binn_list_add_float(struct binn *list, float value) {
	return binn_list_add(list, BINN_FLOAT32, &value, 0);
}

static inline BOOL binn_list_add_double(struct binn *list, double value) {
	return binn_list_add(list, BINN_FLOAT64, &value, 0);
}

static inline BOOL binn_list_add_bool(struct binn *list, BOOL value) {
	return binn_list_add(list, BINN_BOOL, &value, 0);
}

static inline BOOL binn_list_add_null(struct binn *list) {
	return binn_list_add(list, BINN_NULL, NULL, 0);
}

static inline BOOL binn_list_add_str(struct binn *list, char *str) {
	return binn_list_add(list, BINN_STRING, str, 0);
}

static inline BOOL binn_list_add_blob(struct binn *list, void *ptr, int size) {
	return binn_list_add(list, BINN_BLOB, ptr, size);
}
static inline BOOL binn_list_add_list(struct binn *list, void *list2) {
	return binn_list_add(list, BINN_LIST, binn_ptr(list2), binn_size(list2));
}

static inline BOOL binn_list_add_map(struct binn *list, void *map) {
	return binn_list_add(list, BINN_MAP, binn_ptr(map), binn_size(map));
}

static inline BOOL binn_list_add_object(struct binn *list, void *obj) {
	return binn_list_add(list, BINN_OBJECT, binn_ptr(obj), binn_size(obj));
}

static inline BOOL binn_list_add_value(struct binn *list, struct binn *value) {
	return binn_list_add(list, value->type, binn_ptr(value), binn_size(value));
}

/*************************************************************************************/

static inline BOOL binn_map_set_int8(struct binn *map, int id, signed char value) {
	return binn_map_set(map, id, BINN_INT8, &value, 0);
}

static inline BOOL binn_map_set_int16(struct binn *map, int id, short value) {
	return binn_map_set(map, id, BINN_INT16, &value, 0);
}

static inline BOOL binn_map_set_int32(struct binn *map, int id, int value) {
	return binn_map_set(map, id, BINN_INT32, &value, 0);
}

static inline BOOL binn_map_set_int64(struct binn *map, int id, int64 value) {
	return binn_map_set(map, id, BINN_INT64, &value, 0);
}

static inline BOOL binn_map_set_uint8(struct binn *map, int id, unsigned char value) {
	return binn_map_set(map, id, BINN_UINT8, &value, 0);
}

static inline BOOL binn_map_set_uint16(struct binn *map, int id, unsigned short value) {
	return binn_map_set(map, id, BINN_UINT16, &value, 0);
}

static inline BOOL binn_map_set_uint32(struct binn *map, int id, unsigned int value) {
	return binn_map_set(map, id, BINN_UINT32, &value, 0);
}

static inline BOOL binn_map_set_uint64(struct binn *map, int id, uint64 value) {
	return binn_map_set(map, id, BINN_UINT64, &value, 0);
}

static inline BOOL binn_map_set_float(struct binn *map, int id, float value) {
	return binn_map_set(map, id, BINN_FLOAT32, &value, 0);
}

static inline BOOL binn_map_set_double(struct binn *map, int id, double value) {
	return binn_map_set(map, id, BINN_FLOAT64, &value, 0);
}

static inline BOOL binn_map_set_bool(struct binn *map, int id, BOOL value) {
	return binn_map_set(map, id, BINN_BOOL, &value, 0);
}

static inline BOOL binn_map_set_null(struct binn *map, int id) {
	return binn_map_set(map, id, BINN_NULL, NULL, 0);
}

static inline BOOL binn_map_set_str(struct binn *map, int id, char *str) {
	return binn_map_set(map, id, BINN_STRING, str, 0);
}

static inline BOOL binn_map_set_blob(struct binn *map, int id, void *ptr, int size) {
	return binn_map_set(map, id, BINN_BLOB, ptr, size);
}

static inline BOOL binn_map_set_list(struct binn *map, int id, void *list) {
	return binn_map_set(map, id, BINN_LIST, binn_ptr(list), binn_size(list));
}

static inline BOOL binn_map_set_map(struct binn *map, int id, void *map2) {
	return binn_map_set(map, id, BINN_MAP, binn_ptr(map2), binn_size(map2));
}

static inline BOOL binn_map_set_object(struct binn *map, int id, void *obj) {
	return binn_map_set(map, id, BINN_OBJECT, binn_ptr(obj), binn_size(obj));
}

static inline BOOL binn_map_set_value(struct binn *map, int id, struct binn *value) {
	return binn_map_set(map, id, value->type, binn_ptr(value), binn_size(value));
}

/*************************************************************************************/

static inline BOOL binn_object_set_int8(struct binn *obj, char *key, signed char value) {
	return binn_object_set(obj, key, BINN_INT8, &value, 0);
}

static inline BOOL binn_object_set_int16(struct binn *obj, char *key, short value) {
...
static inline BOOL binn_object_set_int64(struct binn *obj, char *key, int64 value) {
...
static inline BOOL binn_object_set_uint16(struct binn *obj, char *key,
...
static inline BOOL binn_object_set_float(struct binn *obj, char *key, float value) {
	return binn_object_set(obj, key, BINN_FLOAT32, &value, 0);
}

static inline BOOL binn_object_set_double(struct binn *obj, char *key, double value) {
	return binn_object_set(obj, key, BINN_FLOAT64, &value, 0);
}

static inline BOOL binn_object_set_bool(struct binn *obj, char *key, BOOL value) {
	return binn_object_set(obj, key, BINN_BOOL, &value, 0);
}

static inline BOOL binn_object_set_null(struct binn *obj, char *key) {
	return binn_object_set(obj, key, BINN_NULL, NULL, 0);
}

static inline BOOL binn_object_set_str(struct binn *obj, char *key, char *str) {
...
static inline BOOL binn_object_set_list(struct binn *obj, char *key, void *list) {
	return binn_object_set(obj, key, BINN_LIST, binn_ptr(list), binn_size(list));
}

static inline BOOL binn_object_set_map(struct binn *obj, char *key, void *map) {
	return binn_object_set(obj, key, BINN_MAP, binn_ptr(map), binn_size(map));
}

static inline BOOL binn_object_set_object(struct binn *obj, char *key, void *obj2) {
	return binn_object_set(obj, key, BINN_OBJECT, binn_ptr(obj2), binn_size(obj2));
}

static inline BOOL binn_object_set_value(struct binn *obj, char *key, struct binn *value) {
...
static inline BOOL binn_object_get_int8(void *obj, char *key, signed char *pvalue) {
	return binn_object_get(obj, key, BINN_INT8, pvalue, NULL);
}

static inline BOOL binn_object_get_int16(void *obj, char *key, short *pvalue) {
...
static inline BOOL binn_object_get_int64(void *obj, char *key, int64 *pvalue) {
...
static inline BOOL binn_object_get_uint16(void *obj, char *key, unsigned short *pvalue) {
...
static inline BOOL binn_object_get_float(void *obj, char *key, float *pvalue) {
	return binn_object_get(obj, key, BINN_FLOAT32, pvalue, NULL);
}

static inline BOOL binn_object_get_double(void *obj, char *key, double *pvalue) {
	return binn_object_get(obj, key, BINN_FLOAT64, pvalue, NULL);
}

static inline BOOL binn_object_get_bool(void *obj, char *key, BOOL *pvalue) {
	return binn_object_get(obj, key, BINN_BOOL, pvalue, NULL);
}

static inline BOOL binn_object_get_str(void *obj, char *key, char **pvalue) {
...
static inline BOOL binn_object_get_list(void *obj, char *key, void **pvalue) {
	return binn_object_get(obj, key, BINN_LIST, pvalue, NULL);
}

static inline BOOL binn_object_get_map(void *obj, char *key, void **pvalue) {
	return binn_object_get(obj, key, BINN_MAP, pvalue, NULL);
}

static inline BOOL binn_object_get_object(void *obj, char *key, void **pvalue) {

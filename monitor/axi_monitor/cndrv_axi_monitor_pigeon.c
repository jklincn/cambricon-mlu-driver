...
extern struct cn_aximhub_data_parse_ops aximhub_pigeon_edge_parse_ops;

#if defined(CONFIG_CNDRV_PIGEON_SOC)

#include <linux/soc/cambricon/irqs.h>
#define PIGEON_AXIMHUB_0_IRQ         (DECLARE_SPI_WITH_FLAGS(289, IRQ_TYPE_LEVEL_HIGH))
#define PIGEON_AXIMHUB_1_IRQ         (DECLARE_SPI_WITH_FLAGS(290, IRQ_TYPE_LEVEL_HIGH))

#else
#define PIGEON_AXIMHUB_0_IRQ         (0)
#define PIGEON_AXIMHUB_1_IRQ         (0)
#endif

irqreturn_t aximhub_pigeon_intr_handle(int index, void *data);

#define PIGEON_HUB0_MONITOR_CNT 16
#define PIGEON_HUB1_MONITOR_CNT 16

struct cn_axi_monitor_config pigeon_config[PIGEON_SINGLE_DIE_MAX_AXI_MON_NUM] = {
	{0, PIGEON_HUB0_MONITOR_CNT,
		0x9F000, aximhub_pigeon_intr_handle, 0x3F5F, 0x8000},
	{0, PIGEON_HUB1_MONITOR_CNT,
		0x8000, aximhub_pigeon_intr_handle, 0x1FFF, 0x8000}
};

int aximhub_pigeon_edge_config_intr_irq(void)
{
	pigeon_config[0].irq = PIGEON_AXIMHUB_1_IRQ;
	pigeon_config[1].irq = PIGEON_AXIMHUB_0_IRQ;
	return 0;
}

struct cn_aximon_zone_info pigeon_zone_info = {
...
struct cn_aximon_zone_info pigeon_d2d_zone_info = {
	/*interrupt is trigged every zone size bytes*/
	ZONE_SIZE_8MB,
	ZONE_CONUT,
	/*device buffer size*/
...
int axihub_pigeon_stop_hub(struct cambr_amh_hub *axi_set)
{
	struct cn_core_set *core = (struct cn_core_set *)axi_set->core;
	u32 reg32 = 0;
	int ret = 0;
...
int axihub_pigeon_start_hub(struct cambr_amh_hub *axi_set)
{
	struct cn_core_set *core = (struct cn_core_set *)axi_set->core;
	u32 reg32 = 0;
...
struct cn_aximhub_ops aximon_pigeon_ops = {
	.stop_hub = axihub_pigeon_stop_hub,
	.start_hub = axihub_pigeon_start_hub,
};

void pigeon_axi_monitor_config(void *p)
{
	struct cn_monitor_set *monitor_set = (struct cn_monitor_set *)p;
	u64 *res_param = monitor_set->res_param;

	monitor_set->hub_num = PIGEON_SINGLE_DIE_MAX_AXI_MON_NUM;
	monitor_set->zone_info = &pigeon_zone_info;
	monitor_set->config = pigeon_config;
	monitor_set->ops = aximon_pigeon_ops;
	monitor_set->highrate_mode = AXI_MONITOR_MATCH_ALL_MODE;
	monitor_set->parse_ops = &aximhub_pigeon_edge_parse_ops;
	monitor_set->monitor_ops = &aximon_mlu300_ops;
	aximhub_pigeon_edge_config_intr_irq();

...
#include "../include/vhost/vringh.h"
...
static int __maybe_unused vringh_dump_vring(const struct vringh *vrh)
{
	int i;

	pr_info("\n============= vring=============");
	pr_info("num: %d\n", vrh->vring.num);
	pr_info("last_used_idx: %d\n", vrh->last_used_idx);
	pr_info("last_avail_idx: %d\n", vrh->last_avail_idx);
	pr_info("completed: %d\n", vrh->completed);

	pr_info("======== desc =========\n");
	for (i = 0; i < vrh->vring.num; i++) {
		pr_info("desc[%d]: flag %d, len %d, addr 0x%llx, next %d\n",
			i, vrh->vring.desc[i].flags, vrh->vring.desc[i].len,
			vrh->vring.desc[i].addr, vrh->vring.desc[i].next);
	}
	pr_info("======== avail =========\n");
	for (i = 0; i < vrh->vring.num; i++) {
		pr_info("avail: ring[%d] = %d\n", i, vrh->vring.avail->ring[i]);
	}
	pr_info("vring_avail_event: %d\n", vring_avail_event(&vrh->vring));
	pr_info("avail: flags %d,  idx %d\n",
					vrh->vring.avail->flags, vrh->vring.avail->idx);
	pr_info("======== used =========\n");
	if (0) {//avoid read outbound
		for (i = 0; i < vrh->vring.num; i++) {
			pr_info("used: ring[%d].id = %d, ring[%d].len = %d\n",
				i, vrh->vring.used->ring[i].id, i, vrh->vring.used->ring[i].len);
		}
		pr_info("vring_used_event: %d\n",	vring_used_event(&vrh->vring));
		pr_info("used: flags %d,  idx %d\n",
						vrh->vring.used->flags, vrh->vring.used->idx);
	}
				    ...
				    u16 *last_avail_idx)
{
	...
	if (err) {
		vringh_bad("Failed to access avail idx at %px",
			   &vrh->vring.avail->idx);
		return err;
	}
	...
	if (err) {
		vringh_bad("Failed to read head: idx %d address %px",
						...
						u64, struct vringh_range *))
{
	...
	WARN_ON(addr < range->start || addr > range->end_incl);
			   ...
			   void *dst, const void *src, size_t len))
{
	...
	for (;;) {
		...
		if (count++ == vrh->vring.num) {
			vringh_bad("Descriptor loop in %px", descs);
			err = -ELOOP;
			goto fail;
		}
		...
		if (desc.flags & cpu_to_vringh16(vrh, VRING_DESC_F_WRITE)) {
			...
			if (unlikely(wiov && wiov->i)) {
				vringh_bad("Readable desc %px after writable",
					   &descs[i]);
				err = -EINVAL;
				goto fail;
			}
						   ...
						   *src, unsigned int num))
{
	...
	if (err) {
		vringh_bad("Failed to write %u used entries %u at %px",
	...
	if (err) {
		vringh_bad("Failed to update used index at %px",
			   &vrh->vring.used->idx);
		return err;
	}
						     ...
						     const __virtio16 *p))
{
	...
	if (!vrh->event_indices) {
		...
		if (err) {
			vringh_bad("Failed to get flags at %px",
	...
	if (err) {
		vringh_bad("Failed to get used event idx at %px",
			   &vring_used_event(&vrh->vring));
		return err;
	}
							...
							__virtio16 *p, u16 val))
{
	...
	if (!vrh->event_indices) {
		...
		if (putu16(vrh, &vrh->vring.used->flags, 0) != 0) {
			vringh_bad("Clearing used flags %px",
		...
		if (putu16(vrh, &vring_avail_event(&vrh->vring),
			...
			vringh_bad("Updating avail event index %px",
				   &vring_avail_event(&vrh->vring));
			return true;
		}
	}
	...
	if (getu16(vrh, &avail, &vrh->vring.avail->idx) != 0) {
		vringh_bad("Failed to check avail idx at %px",
			   &vrh->vring.avail->idx);
		return true;
	}
							 ...
							 __virtio16 *p, u16 val))
{
	if (!vrh->event_indices) {
		...
		if (putu16(vrh, &vrh->vring.used->flags,
			...
			vringh_bad("Setting used flags %px",
				   &vrh->vring.used->flags);
		}
	}
}

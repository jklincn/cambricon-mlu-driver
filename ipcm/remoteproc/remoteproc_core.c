...
phys_addr_t rproc_va_to_pa(struct rproc *rproc, void *cpu_addr)
{
	...
	if (rproc->ops->va_to_pa) {
		pa = rproc->ops->va_to_pa(rproc, (u64)cpu_addr);
		if (pa)
			return pa;
		//go on for further check
	}
	/* cambricon add end */
	/*
				   ...
				   struct rproc_mem_entry *mem, u64 da, u32 len)
{
	...
	dev_dbg(dev, "%s, mem:%s len(%d), da(0x%llx), mem->da(0x%llx)\n", __func__, mem->name, len, da, mem->da);
...
int rproc_alloc_vring(struct rproc_vdev *rvdev, int i)
{
	...
	dev_info(dev, "%s, rvring->len(%d), rvring->align(%d), vring_size(%d), size(%zu); rsc->vring[%d] da = 0x%llx\n",
		__func__, rvring->len, rvring->align, vring_size(rvring->len, rvring->align),
		size, i, rsc->vring[i].da);
	...
	if (mem) {
		if (rproc_check_carveout_da(rproc, mem, rsc->vring[i].da, size)) {
			dev_err(dev, "rproc_check_carveout_da failed!\n");
			return -ENOMEM;
		}
	} else {
...
static int rproc_vdev_do_start(struct rproc_subdev *subdev)
{
	...
	#else
	return rproc_add_vhost_dev(rvdev, rvdev->id);
	#endif
}
...
static void rproc_vdev_do_stop(struct rproc_subdev *subdev, bool crashed)
{
	...
	#else
	ret = device_for_each_child(&rvdev->dev, NULL, rproc_remove_vhost_dev);
	#endif
	if (ret)
			     ...
			     int offset, int avail)
{
	...
	dev_info(dev, "vdev rsc: id %d, dfeatures 0x%x, cfg len %d, %d vrings\n",
	...
	snprintf(name, sizeof(name), "vdev%dbuffer", rproc_get_rvdev_index(rproc));
	rvdev->dev.parent = &rproc->dev;
	//#if (KERNEL_VERSION(3, 16, 0) <= LINUX_VERSION_CODE) && (KERNEL_VERSION(5, 10, 0) > LINUX_VERSION_CODE)
	...
	//cambricon FIXME arm64 4.9 have no set_dma_ops
			      ...
			      int offset, int avail)
{
	...
	dev_info(dev, "%s added: da 0x%llx, len 0x%x\n",
		name, rsc->da, rsc->len);
			       ...
			       int offset, int avail)
{
	...
	dev_info(dev, "mapped devmem pa 0x%llx, da 0x%llx, len 0x%x\n",
				...
				struct rproc_mem_entry *mem)
{
	...
	dev_info(dev, "carveout va %px, dma %pad, len 0x%zx\n",
		va, &dma, mem->len);
	...
	if (mem->da != FW_RSC_ADDR_ANY && rproc->domain) {
		...
		dev_info(dev, "carveout mapped 0x%llx to %pad\n",
			mem->da, &dma);
	}
				 ...
				 int offset, int avail)
{
	...
	dev_info(dev, "carveout rsc: name: %s, da 0x%llx, pa 0x%llx, len 0x%x, flags 0x%x\n",
...
static int rproc_alloc_registered_carveouts(struct rproc *rproc)
{
	...
	list_for_each_entry_safe(entry, tmp, &rproc->carveouts, node) {
		...
		/* carveout mem is from rsc table, not by rproc_add_carveout() */
		if (entry->rsc_offset != FW_RSC_ADDR_ANY) {
			pr_info("%s, %d, update resource table\n", __func__, __LINE__);
			...
			if (entry->va)
				pa = (u64)rproc_va_to_pa(rproc, entry->va);
			else
				pa = (u64)entry->dma;
...
static int rproc_start(struct rproc *rproc, const struct firmware *fw)
{
	...
	if (loaded_table) {
		/* arm not override rsc table cause outbound magic inside */
		if (!rproc_get_role(rproc))
...
/* cambricon */
int rproc_dummy_boot(struct rproc *rproc);
...
static int rproc_trigger_auto_boot(struct rproc *rproc)
{
	...
	/* cambricon
	 * fake boot, but we still need some processes without real powerup load&boot firmware
	 */
	if (0) {
		/*
 ...
 * @rproc: handle of a remote processor
 *
 * fake boot a remote processor without handle it's firmware & power
 *
 * Returns 0 on success, and an appropriate error value otherwise.
 */
int rproc_dummy_boot(struct rproc *rproc)
...
static inline int _devm_add_action_or_reset(struct device *dev,
...
int devm_rproc_add(struct device *dev, struct rproc *rproc)
{
	...
	return _devm_add_action_or_reset(dev, devm_rproc_remove, rproc);
}
			  ...
			  const char *firmware, int len)
{
	...
	dev_set_drvdata(&rproc->dev, rproc);
...
static void rproc_init_panic(void)
{
	rproc_panic_nb.notifier_call = rproc_panic_handler;
	atomic_notifier_chain_register(&panic_notifier_list, &rproc_panic_nb);
}

static void rproc_exit_panic(void)
{

...
static void force_dependecies(struct perftest_parameters *user_param)
{
	...
	#ifdef HAVE_MLU
	if (user_param->use_mlu && user_param->mmap_file != NULL) {
		printf(RESULT_LINE);
		fprintf(stderr,"You cannot use MLU and an mmap'd file at the same time\n");
		exit(1);
	}
	#endif

...
static int pp_init_mlu(struct pingpong_context *ctx, int mlu_device_id)
{
	int mlu_pci_bus_id;
...
static void pp_free_mlu(struct pingpong_context *ctx)
{
		...
		struct perftest_parameters *user_param)
{
	...
	#endif
	#ifdef HAVE_MLU
	if (user_param->use_mlu) {
		for (i = 0; i < dereg_counter; i++) {
			CNaddr mlu_addr = (CNaddr)ctx->buf[i];
...
int create_single_mr(struct pingpong_context *ctx, struct perftest_parameters *user_param, int qp_index)
{
	...
	if (user_param->use_mlu) {
		...
		ctx->buf[qp_index] = (void *)mlu_addr;
	} else
	#endif
...
int ctx_init(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	#ifdef HAVE_MLU
	if (user_param->use_mlu) {
		if (pp_init_mlu(ctx, user_param->mlu_device_id)) {
			fprintf(stderr, "Couldn't initialize MLU device\n");
			goto mkey;
		}
	}
	#endif

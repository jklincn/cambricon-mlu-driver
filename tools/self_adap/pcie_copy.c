...
{
	printf("Usage:  pcie_copy Test [OPTION]...\n");
	printf("\n");
	printf("Options:\n");
	...
	printf("--noaligned=[FLAG]\tdefault:FLAG=1 enable noaligned_test FLAG=0 disable noaligned\n");
	...
	printf("\nExample:\n");
	printf("./pcie_copy --device=0,1 --thread=10 --repeat=10 --dma_mode=sync\n");
	printf("./pcie_copy --device=0 --memory=pinned --thread=10 --repeat=10 --noaligned=0\n");
	printf("./pcie_copy --device=0 --memory=pageable --thread=10 --repeat=10 --mode=detail\n");
}
...
{
	printf("host_addr A:%#lx  host_addr B:%#lx  device_addr C:%#lx\tsize:%#lx\n",
			(uint64_t)info->host_addr_0,
			(uint64_t)info->host_addr_1,
			info->dev_addr_0,
			info->size);
	printf("A1=A+offsetA:%#lx  B1=B+offsetB:%#lx  C1=C+offsetC:%#lx\tlen:%#lx\n",
...
int alloc_addr_memory(struct dma_test_struct *info) {
	if (info->cmd.noaligned) {
		...
		info->size = DEFAULT_SIZE + (rand() % (DEFAULT_SIZE / ALIGNED_SIZE)) * ALIGNED_SIZE;
	}
	if (info->cmd.mem_mode == PINNED) {
		...
		info->host_addr_0 = memalign(4096, info->size);
		info->host_addr_1 = memalign(4096, info->size);
	}
...
void free_addr_memory(struct dma_test_struct *info) {
	if (info->cmd.mem_mode == PINNED) {
		...
		if (info->host_addr_0) {
			free(info->host_addr_0);
			info->host_addr_0 = NULL;
		}
		if (info->host_addr_1) {
			free(info->host_addr_1);
			info->host_addr_1 = NULL;
		}
	}
	if (info->dev_addr_0) {
...
int prepare_pcie_copy(struct dma_test_struct *info, struct dma_bw_struct *bw_set)
{
	unsigned long i;
	unsigned long split;
	...
	if (info->cmd.noaligned) {
		...
		split = rand() % (DEFAULT_SIZE / ALIGNED_SIZE - 2) + 1;
		for (i = 0; i < 3; i++) {
			offset[i] = rand() % (DEFAULT_SIZE / ALIGNED_SIZE - split) * ALIGNED_SIZE;
		}
		bw_set->size = split * ALIGNED_SIZE;
	}
	bw_set->dev_addr_0 = info->dev_addr_0 + offset[0];
	bw_set->host_addr_0 = info->host_addr_0 + offset[1];
	bw_set->host_addr_1 = info->host_addr_1 + offset[2];
...
void *sync_dma_thread(void *arg)
{
	...
	for (i = 0; i < info->cmd.repeat_num; i++) {
		info->repeat_id = i + 1;
		bw_set->result = 0;
		if (prepare_pcie_copy(info, bw_set)) {
			return NULL;
		}
		MLU_CHECK(h2d(info->fd, bw_set->dev_addr_0, bw_set->host_addr_0, bw_set->size));
		MLU_CHECK(d2h(info->fd, bw_set->dev_addr_0, bw_set->host_addr_1, bw_set->size));
...
int run_pcie_copy_test(struct dma_test_struct *info)
{
	int i;
	char out_str[128];
	...
	if (info->cmd.dma_mode == ALL_MODE || info->cmd.dma_mode == ASYNC_MODE) {
		...
		for (i = 0; i < info->cmd.thread_num; i++) {
			...
			sprintf(out_str, "repeat:%d  thread:%d async pcie copy ", th_info[i].repeat_id, th_info[i].thread_id);
			if ((info->cmd.mode == DETAIL_MODE &&
	...
	if (info->cmd.dma_mode == ALL_MODE || info->cmd.dma_mode == SYNC_MODE) {
		...
		for (i = 0; i < info->cmd.thread_num; i++) {
			pthread_join(th_info[i].th, NULL);
			sprintf(out_str, "repeat:%d  thread:%d sync pcie copy ", th_info[i].repeat_id, th_info[i].thread_id);
			if ((info->cmd.mode == DETAIL_MODE &&
						detail_output(&th_info[i], &bw_set[i], out_str)) ||
					bw_set[i].result) {
				result = 1;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < run_device_count; i++) {
		...
		if(run_pcie_copy_test(&info[i])) {
			if (info[i].cmd.mode != NO_CHECK_MODE) {
				printf("================================\n");
				printf("device[%d] pcie copy ERROR\n", run_device[i]);
			}
		} else {
			if (info[i].cmd.mode != NO_CHECK_MODE) {
				printf("================================\n");
				printf("device[%d] pcie copy SUCCESS\n", run_device[i]);
			}
		}
	}
